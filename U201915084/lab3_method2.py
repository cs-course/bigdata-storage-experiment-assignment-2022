...
num_samples = 200
wait_time=0 #相邻两次对冲请求发起的时间间隔，若wait_time=0即为关联请求
...
def bench_put(i): 
    ...
    def tied_request():
        def func():
            ...
            os.kill(os.getpid(), signal.SIGTERM) #注意：本程序只能在linux系统下运行才能实现正确功能！！
        # while True:
        for _ in range(request_num):
            t1=threading.Thread(target=func)
            t1.start()
            time.sleep(wait_time)     
        time.sleep(100000) #等待充分长时间，使得在等待过程中进程被结束，然后退出
            
    
    p=multiprocessing.Process(target=tied_request)
    start = time.time()*1000 # 换算为毫秒
    p.start()
    p.join()
    end = time.time()*1000
    # print(threading.current_thread().name,'end',end)
    duration = end - start
    # print(duration)
...
def run_test(test_type):
    ...
    with tqdm(desc="Accessing S3", total=num_samples) as pbar:
        with ProcessPoolExecutor(max_workers=num_clients) as executor: # 通过 max_workers 设置并发线程数
            futures = [
                executor.submit(
                    switch[test_type],
    ...
    tracefile_name = 'sig_'+str(wait_time)+'_'+str(request_num)+'_'+test_type+'_'+str(object_size)+'_'+str(num_clients)+'_'+str(num_samples)+'.csv'
